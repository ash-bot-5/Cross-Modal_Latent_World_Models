"""
Pre-cache raw episode frames as memory-mappable .npy files -- langtable_switch variant.

Same as cache_frames_npy.py, except DATA_DIR points at the langtable_switch episode set
(suboptimal/failure-heavy trajectories) instead of the main langtable/demos/ set.
...
lifetime — fine for small subsets, but OOMs across a full episode set. This script
stacks each episode's frames into a single (T,180,320,3) uint8 array and saves it as a
plain (uncompressed) .npy file, so downstream code can do
`np.load(path, mmap_mode="r")[t]` to read a single frame's bytes on demand without ever
materializing the whole episode in memory. Plain .npy (not .npz) is required
specifically because .npz archives don't support mmap_mode on read.

Idempotent — skips episodes whose {stem}_frames.npy already exists.

Usage:
    python cache_frames_npy_switch.py             # cache all episodes
    python cache_frames_npy_switch.py --smoke     # process 3 episodes, verify correctness
"""
...
DATA_DIR = "/home/ashwink/full_data_5000/langtable_switch/demos/"
