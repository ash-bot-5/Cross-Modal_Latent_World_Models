"""
Cache compound z_sem vectors for all timesteps of all episodes.
...
Run:
    ...
    python main_project/caching/full_5000_cache_zsem.py
"""
...
for pkl_path in tqdm(pkl_files, desc="Episodes"):
    ...
    try:
        ...
        if len(peg_qa_list) < T:
            tqdm.write(f"WARN {stem}: peg_touching_starting_block_qa has {len(peg_qa_list)} entries, expected {T}. Skipping.")
            n_failed += 1
            continue
