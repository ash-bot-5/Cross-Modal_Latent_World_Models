...
class LangTableImageDataset(LangTableDataset):
    """LangTableDataset variant for CLIP-image-encoder fine-tuning. Stores raw
    frames (not cached frozen z_obs) plus a shared deterministic CLIP preprocess
    transform, applying ONLY that transform (no model, no grad) in __getitem__.
    Safe with num_workers > 0 — no GPU-resident nn.Module lives on the Dataset.
    """
...
def build_probe_episode_records(data_dir: str, stems: list[str]) -> dict[str, dict]:
    """For each episode stem: load raw frames + cached touching label + ground-truth
    task-relevant block distance dist_ab(t), same formula as cache_zhard.py."""
    records = {}
    for stem in stems:
...
def encode_probe_episodes(
    ...
    with torch.no_grad():
        for stem, rec in records.items():
            frames = rec["frames"]
            T = len(frames)
            imgs = torch.stack([preprocess(Image.fromarray(frames[t])) for t in range(T)])
            z_parts = []
...
def plot_progress_curves(
    ...
    for stem in episode_stems_to_plot:
        ...
        fig.tight_layout()
        plt.savefig(os.path.join(out_dir, f"{checkpoint_tag}_{stem}.png"), dpi=150)
        plt.close(fig)
...
def train(
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        plot_progress_curves(encoded, per_episode_centroids, plot_stems,
        ...
        print(f"[epoch {epoch}] train_loss={avg_train_loss:.4f} val_loss={avg_val_loss:.4f} "
              f"margin_subsample75={margin_agg['mean']:.4f} progress_corr={progress_corr:.4f}")
