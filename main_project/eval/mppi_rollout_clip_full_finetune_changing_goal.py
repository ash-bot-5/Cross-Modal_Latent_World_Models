...
jointly fine-tuned dynamics model in LangTable -- TWO-STAGE GOAL variant.

Same as mppi_rollout_clip_full_finetune.py (same epochs 6-15 sweep over the
checkpoints_clip_full_finetuned_wider_hl checkpoints, same env/peg-forcing setup,
same diagnostics), except:
  - steps [0, STAGE_SWITCH_STEP) target an easier stage-1 goal ("A is NOT touching
    B, peg is touching A") instead of the final goal from step 0 -- testing whether
    staging the goal this way helps planning vs. targeting "touching" immediately.
  - steps [STAGE_SWITCH_STEP, MAX_STEPS) switch to the real stage-2 goal ("A is
    touching B, peg is touching A"), same as the single-goal script's fixed goal.
  - mppi.reset() is called at the exact step of the switch, clearing the
    cross-step warm-start state so the next command() call starts a fresh search
    for the new objective instead of being seeded from a chunk optimized for the
    old one.
  - rollouts are saved to a separate output folder, rollouts_wider_hl_changing_goal/,
    so they don't clobber the single-goal rollouts_wider_hl/ outputs.

At each env timestep:
  1. Encode current frame with the fine-tuned CLIP image tower -> z_t (768,)
  2. Pick z_star (stage 1 or stage 2, depending on step) and call
     mppi.command(z_t) -> action_chunk (16,) normalized in [-1, 1]
...
Run (produces one rollout video per checkpoint in CHECKPOINTS below -- epochs
6-15 of the widened-hidden-layer checkpoints_clip_full_finetuned_wider_hl/ --
saved under main_project/rollouts/rollouts_wider_hl_changing_goal/):
    source /home/ashwink/Documents/swms/.venv/bin/activate
    python main_project/eval/mppi_rollout_clip_full_finetune_changing_goal.py
...
STAGE_SWITCH_STEP = 30  # steps [0, STAGE_SWITCH_STEP) target the stage-1 (not-touching)
                        # goal, steps [STAGE_SWITCH_STEP, MAX_STEPS) target stage-2 (touching)
...
OUT_DIR      = os.path.join(MAIN_PROJECT_DIR, "rollouts", "rollouts_wider_hl_changing_goal")
...
CHECKPOINTS = [
    (f"full_finetune_wider_hl_epoch{epoch}", os.path.join(CKPT_DIR, f"epoch_{epoch:03d}.pt"))
...
def run_rollout(label: str, ckpt_path: str) -> None:
    ...
    # -----------------------------------------------------------------------
    # 1. Two-stage goal text: steps [0, STAGE_SWITCH_STEP) target "not touching"
    # (peg still touching A), then switch to "touching" for the rest -- testing
    # whether staging the goal this way helps planning vs. targeting "touching"
    # from step 0.
    # -----------------------------------------------------------------------
    goal_text_stage1 = (
        f"the {BLOCK_A.replace('_', ' ')} is not touching the {BLOCK_B.replace('_', ' ')}. "
    ...
    log.info(f"Goal (stage 1, steps 0-{STAGE_SWITCH_STEP - 1}): {goal_text_stage1}")
    log.info(f"Goal (stage 2, steps {STAGE_SWITCH_STEP}-{MAX_STEPS - 1}): {goal_text_stage2}\n")
    ...
    # z_star_stage{1,2} use THIS checkpoint's own fine-tuned text tower (both towers
    # were fine-tuned jointly here, so the goal embedding must come from the same
    # model instance -- a separately-loaded frozen CLIP text tower would not match
    # this checkpoint's fine-tuned embedding space). Both encoded in one batched call.
    with torch.no_grad():
        tokens = clip_tokenizer([goal_text_stage1, goal_text_stage2]).to(DEVICE)
        z_star_both = F.normalize(clip_model.encode_text(tokens), dim=-1)  # (2, 768)
    z_star_stage1, z_star_stage2 = z_star_both[0], z_star_both[1]
    log.info(f"z_star shape: {z_star_stage1.shape}\n")
    ...
    def cost_fn(terminal_state):
        # terminal_state: (K, 768), z_star: (768,) -- z_star is looked up from the
        # enclosing scope at call time, so it reflects whichever stage is active
        # for the step currently being planned.
    ...
    with torch.no_grad():
        ...
        for step in range(MAX_STEPS):
            # Two-stage goal: switch from "not touching" to "touching" at
            # STAGE_SWITCH_STEP. mppi.reset() clears the cross-step warm-start
            # state at the exact moment the goal changes -- otherwise the next
            # command() call would be seeded from a chunk optimized for the OLD
            # (stage-1) objective instead of starting a fresh search for stage 2.
            z_star = z_star_stage1 if step < STAGE_SWITCH_STEP else z_star_stage2
            if step == STAGE_SWITCH_STEP:
                log.info(f"\n>>> Goal switch at step {step}: now targeting '{goal_text_stage2}'\n")
                mppi.reset()
