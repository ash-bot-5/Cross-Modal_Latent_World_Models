"""
Cosine-sim diagnostic plots for the CLIP fine-tuning experiments' checkpoints -- EXPERT
ACTIONS variant of eval_finetuned_checkpoints_cosine_sim.py.

Unlike that script (which feeds all-zero actions as a "does the MLP ignore actions?"
ablation), this one feeds the REAL expert action chunk at each timestep -- using the model
the way it's actually meant to be used: z_pred = model(z_obs[t], actions[t:t+HORIZON]) is a
forecast of the state at t+HORIZON, not of t (see CLAUDE.md's note on this exact alignment
requirement, and eval_3_expert_trajs_zcur_zsem.py's existing "+HORIZON"-shifted precedent).

Consequently:
  - cos_sim(t) = cos(z_pred(t), goal) is plotted at x = t + HORIZON (its forecast target),
    not at x = t.
  - dist(A,B) and dist(A,peg) (ground truth) are plotted at their own native x = t, over the
    full episode range -- NOT sliced to T - HORIZON.
...
Episode selection: same 3 episodes as the zero-actions script (same seed=42), drawn from the
LoRA checkpoint's 750-episode val split (a confirmed strict subset of the full-finetune
checkpoint's 820-episode val split), filtered to genuine touching at the final timestep for
BOTH dist(A,B) and dist(A,peg).

Run:
    source /home/ashwink/Documents/swms/.venv/bin/activate
    python main_project/eval/eval_finetuned_checkpoints_cosine_sim_expert_actions.py
"""
...
OUT_DIR = os.path.join(REPO_ROOT, "eval", "finetuned_checkpoints_cosine_sim_plots_expert_actions")
...
# ---------------------------------------------------------------------------
# 1. Action normalization stats (min/max over EVERY .pkl in DATA_DIR, matching
#    eval_3_expert_trajs_clip_lora.py's existing convention).
# ---------------------------------------------------------------------------

print(f"Computing action stats from {DATA_DIR} ...")
all_actions = []
for pkl_file in sorted(f for f in os.listdir(DATA_DIR) if f.endswith(".pkl")):
...
print(f"  action shape: {all_actions.shape}  |  lo={ACTION_LO.tolist()}  hi={ACTION_HI.tolist()}\n")


# ---------------------------------------------------------------------------
# 2. Episode selection: 3 random episodes from the LoRA 750-episode val split,
#    filtered to genuine A-B touching AND peg-A touching at the final timestep.
#    (Identical logic/seed to eval_finetuned_checkpoints_cosine_sim.py -> same 3 episodes.)
# ---------------------------------------------------------------------------
...
for tag, ckpt_path, kind in CHECKPOINTS:
    ...
    for stem in episode_stems:
        ...
        # cos_sim(t) is a forecast of the state at t + HORIZON -- plotted at that x-coordinate.
        timesteps_pred, cosine_sims = [], []
        with torch.no_grad():
            for t in range(T - HORIZON):
                ...
                actions_norm = ((chunk - ACTION_LO) / (ACTION_HI - ACTION_LO) * 2.0 - 1.0).unsqueeze(0)

                z_pred = model(z_t, actions_norm).squeeze(0)
                cos_sim = F.cosine_similarity(z_pred.unsqueeze(0), goal.unsqueeze(0)).item()
                timesteps_pred.append(t + HORIZON)
                cosine_sims.append(cos_sim)

        timesteps_pred = np.array(timesteps_pred, dtype=np.int64)
        cosine_sims = np.array(cosine_sims, dtype=np.float32)

        # ground-truth distance curves cover the FULL episode range, not sliced to T - HORIZON
        timesteps_gt = np.arange(T, dtype=np.int64)
        dist_ab = np.array(
            [np.linalg.norm(block_states[t][a] - block_states[t][b]) for t in timesteps_gt],
            dtype=np.float32,
        )
        dist_peg = np.array(
            [np.linalg.norm(peg_positions[t] - block_states[t][a]) for t in timesteps_gt],
            dtype=np.float32,
        )

        # ---- plot ----
        fig, ax1 = plt.subplots(figsize=(9, 6))
        ax1.plot(timesteps_pred, cosine_sims, color="tab:blue", marker="o", markersize=3,
                  label="cos_sim(z_pred(t), goal) @ t+HORIZON")
        ax1.set_xlabel("Timestep  (dist curves at t; cos_sim at t+HORIZON, its forecast target)")
        ax1.set_ylabel("Cosine similarity to goal z_sem", color="tab:blue")
        ax1.tick_params(axis="y", labelcolor="tab:blue")

        ax2 = ax1.twinx()
        ax2.plot(timesteps_gt, dist_ab, color="tab:red", marker="x", markersize=3, label="dist(A, B)")
        ax2.plot(timesteps_gt, dist_peg, color="tab:purple", marker="^", markersize=3, linestyle="--", label="dist(A, peg)")
        ...
        ax1.set_title(f"{stem} ({tag}, expert actions)\n\"{instruction}\"")
        fig.tight_layout()
