"""
Live MPPI planning loop using the CLIP-LoRA jointly fine-tuned dynamics model in LangTable.

Same as mppi_rollout.py, except:
  - the current-frame CLIP encoder is LoRA-loaded from a checkpoint's lora_state_dict (the
    frozen-CLIP baseline reads model_state_dict; this reads mlp_state_dict + lora_state_dict),
    since the whole point of the CLIP-LoRA experiment was jointly fine-tuning the image encoder.
  - the peg is force-teleported right after env.reset() to start collinear with, and just past,
    BLOCK_A (green_cube) on the side opposite BLOCK_B (blue_moon) — i.e. peg -> green_cube ->
    blue_moon in a straight line, so a single push accomplishes the task instead of needing to
    navigate first.

At each env timestep:
  1. Encode current frame with LoRA-loaded CLIP -> z_t (768,)
...
in CHECKPOINTS below):
    ...
    python main_project/eval/mppi_rollout_clip_lora.py
...
# The two CLIP-LoRA checkpoints this script compares — (descriptive label, checkpoint path).
# Labels are used directly in the output .mp4 / log filenames.
CHECKPOINTS = [
    ("clip_lora_epoch0", os.path.join(MAIN_PROJECT_DIR, "training", "checkpoints_clip_image_finetuned", "epoch_000.pt")),
    ("clip_lora_epoch13", os.path.join(MAIN_PROJECT_DIR, "training", "checkpoints_clip_image_finetuned", "epoch_013.pt")),
]
...
def run_rollout(label: str, ckpt_path: str) -> None:
    ...
    # -----------------------------------------------------------------------
    # 2. Checkpoint + CLIP (image + text encoder) with LoRA loaded onto the visual tower
    # -----------------------------------------------------------------------
    log.info(f"Checkpoint: {ckpt_path}")
    ckpt = torch.load(ckpt_path, map_location=DEVICE)
    log.info(f"  epoch={ckpt['epoch']}  step={ckpt.get('step', '?')}\n")

    log.info("Loading CLIP ViT-L/14 (datacomp_xl_s13b_b90k) + LoRA adapter ...")
