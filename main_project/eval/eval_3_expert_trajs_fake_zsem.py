"""
Fake-z_sem InfoNCE diagnostic on 3 expert trajectories (episodes 0, 1, 3).

Same as eval_3_expert_trajs.py (real actions, real per-timestep MLP inference), but at each
timestep the cosine similarity is computed against BOTH the episode's true final-timestep z_sem
goal AND a fixed, clearly-false CLIP text embedding:
...
Run:
    ...
    python main_project/eval/eval_3_expert_trajs_fake_zsem.py
"""
...
# ---------------------------------------------------------------------------
# 2. Encode the fixed fake statement with the same CLIP text tower used for z_sem
#    Checkpoint-independent — computed once, reused for both runs.
# ---------------------------------------------------------------------------
print("Loading OpenCLIP ViT-L/14 (datacomp_xl_s13b_b90k) for fake-statement encoding ...")
...
# ---------------------------------------------------------------------------
# 3. Per-checkpoint, per-episode evaluation
# ---------------------------------------------------------------------------
