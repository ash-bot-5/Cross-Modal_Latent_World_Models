"""
Linear-probe evaluation of the latent DYNAMICS MODEL's forecast z_pred = model(z_t, a_t:t+8),
as opposed to linear_probe_global_table.py / linear_probe_global_table_image.py, which only
probe the CLIP embedder itself (text tower on synthetic statements / image tower on real
frames respectively). This script asks: given a real (s_t, a_t:t+8) sampled from real expert
episodes, how well does z_pred -- the dynamics model's PREDICTION of the future latent -- let
a linear probe recover the actual physical state at t+8?

Ground truth always comes from /home/ashwink/full_data_5000/langtable/demos/ (all 5000 .pkl
there are _success.pkl -- already 100% "expert data", no filtering needed). z_pred forecasts
t+HORIZON (HORIZON=8), so every label/distance is read at t+HORIZON, never at t.

Runs across every epoch checkpoint found in one or more --ckpt-dirs (checkpoint directories
are live/growing -- this script discovers whichever epoch_*.pt files exist at run time rather
than hardcoding a count). z_t is computed by re-encoding the raw frame through EACH
CHECKPOINT'S OWN fine-tuned CLIP image tower (both checkpoint dirs fully fine-tune both CLIP
towers) -- never by reusing the pretrained-encoder _zobs.npz cache. Actions are normalized via
the fixed constant a/0.035, matching both training scripts' LangTableImageTextDataset
(NOT a fitted min/max stat).

Probes:
  Task-relevant (4 charts, task_relevant/, one metric-vs-epoch line plot per probe, one line
  per checkpoint dir):
    1. peg touching start_block at t+8            (classification, val_auc)
    2. task-relevant blocks (A,B) touching at t+8  (classification, val_auc)
    3. dist(peg, start_block) at t+8               (regression, val_r2)
    4. dist(A, B) at t+8                           (regression, val_r2)

  General state (general_state/, one grouped bar chart PER CHECKPOINT-EPOCH SNAPSHOT): for
  each of the 8 blocks, val_auc of "is the peg touching block X at t+8" fit SEPARATELY on
  episodes where X was the episode's start_block vs. episodes where it wasn't -- two disjoint
  probes per block, not one probe on the pooled population, so the chart's start-vs-non-start
  gap reflects a real generalization difference rather than being averaged away (or worse,
  reflecting some shortcut correlated with which population a row came from).

Every plotted number is a VALIDATION-split metric only (val_auc / val_r2). Train metrics are
computed too (train_acc / train_r2) but only logged to results.json for overfitting context,
never plotted.

Run:
    source /home/ashwink/Documents/swms/.venv/bin/activate
    python main_project/eval/linear_probe_MLP_output.py
"""
...
HORIZON = 8
TOUCH_THRESH = 0.05
ACTION_NORM_CONST = 0.035  # fixed-constant rescale, matches LangTableImageTextDataset
...
def parse_args():
    ...
    p.add_argument("--force-recompute", action="store_true",
                    help="Bypass ground-truth and z_pred npz caches and regenerate everything.")
    return p.parse_args()
...
# ---------------------------------------------------------------------------
# Inline model classes -- verbatim from cost_landscape_mppi_selected.py, confirmed to match
# both checkpoint dirs' mlp_state_dict keys exactly (widened hidden dim 1024, Conv1d action
# encoder).
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Block names -- reused pattern from linear_probe_global_table.py
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Ground-truth sampling (checkpoint-independent -- computed once, cached, reused byte-
# identical across every checkpoint/epoch).
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Per-checkpoint z_pred computation
# ---------------------------------------------------------------------------
...
def merge_results(existing: list[dict], new: list[dict]) -> list[dict]:
    """Keyed by (checkpoint_dir_tag, epoch, probe, block, filter) so re-running a script
    invocation against a checkpoint dir NOT included in today's --ckpt-dirs (e.g. running
    wider_hl today, the subopt-data dir tomorrow) accumulates rather than clobbers --
    otherwise the task-relevant comparison charts would only ever show whichever dir was
    passed in the most recent run."""
...
def plot_general_state_bar_chart(block_names: list[str], start_aucs: list, nonstart_aucs: list,
                                  tag: str, epoch: int, out_path: str):
    x = np.arange(len(block_names))
    width = 0.35

    start_vals = [np.nan if v is None else v for v in start_aucs]
    nonstart_vals = [np.nan if v is None else v for v in nonstart_aucs]

    fig, ax = plt.subplots(figsize=(10, 6))
    ax.bar(x - width / 2, start_vals, width, label="block = start_block", color="tab:blue")
    ax.bar(x + width / 2, nonstart_vals, width, label="block != start_block", color="tab:orange")
    ax.axhline(0.5, color="black", linestyle="--", linewidth=1, label="chance (AUC=0.5)")

    ax.set_xticks(x)
    ax.set_xticklabels([b.replace("_", " ") for b in block_names], rotation=30, ha="right")
    ax.set_ylabel("val_auc (peg touching this block at t+8)")
    ax.set_ylim(0.0, 1.05)
    ax.set_title(f"{tag} epoch {epoch}: peg-touching-block probe, by block identity")
...
def main():
    ...
    general_state_dir = os.path.join(args.out_dir, "general_state")
    zpred_cache_dir = os.path.join(args.out_dir, "zpred_cache")
    for d in (task_relevant_dir, general_state_dir, zpred_cache_dir):
    ...
    print("Sampling ground-truth data (checkpoint-independent) ...")
    gt = sample_ground_truth_data(
        DATA_DIR, block_names, args.n_episodes, args.timesteps_per_episode,
        args.val_frac, args.seed, gt_cache_path, args.force_recompute,
    )
    train_mask = ~gt["is_val"]
    val_mask = gt["is_val"]
    print(f"Total rows: {len(gt['frames'])}  (train={train_mask.sum()}, val={val_mask.sum()})")
    print(f"  peg_touch_start positive: {gt['peg_touch_start'].sum()}")
    print(f"  ab_touch positive: {gt['ab_touch'].sum()}\n")

    block_filter_masks = {}
    for b in block_names:
        is_start = gt["start_block"] == b
        block_filter_masks[b] = {"start": is_start, "nonstart": ~is_start}
    ...
    for ckpt_dir in args.ckpt_dirs:
        ...
        for ckpt_path in epoch_paths:
            ...
            z_pred = compute_zpred_for_checkpoint(
                ...
                zpred_cache_path, args.force_recompute,
            )[0]

            base_meta = {"checkpoint_dir_tag": tag, "checkpoint_dir": ckpt_dir, "epoch": epoch}

            r = fit_classification_probe(
                z_pred, gt["peg_touch_start"], train_mask, val_mask,
                {**base_meta, "probe": "peg_touch_start"},
            )
            results.append(r)
            r = fit_classification_probe(
                z_pred, gt["ab_touch"], train_mask, val_mask,
            ...
            start_aucs, nonstart_aucs = [], []
            for block_idx, b in enumerate(block_names):
                y = gt["peg_touch_block"][:, block_idx]
                filt = block_filter_masks[b]
                block_results = {}
                for filt_name in ("start", "nonstart"):
                    fm = filt[filt_name]
                    r = fit_classification_probe(
                        y=y, X=z_pred,
                        train_mask=train_mask & fm, val_mask=val_mask & fm,
                        meta={**base_meta, "probe": "peg_touch_block", "block": b, "filter": filt_name},
                    )
                    results.append(r)
                    block_results[filt_name] = r
                start_aucs.append(block_results["start"].get("val_auc"))
                nonstart_aucs.append(block_results["nonstart"].get("val_auc"))

            bar_path = os.path.join(general_state_dir, f"{tag}_epoch{epoch:03d}_peg_touch_by_block.png")
            plot_general_state_bar_chart(block_names, start_aucs, nonstart_aucs, tag, epoch, bar_path)
            print(f"    Saved bar chart -> {bar_path}")

    results_path = os.path.join(args.out_dir, "results.json")
    results = merge_results(load_existing_results(results_path), results)
    with open(results_path, "w") as f:
        json.dump(results, f, indent=2)
    print(f"\nSaved results table ({len(results)} rows, merged with any prior run) -> {results_path}")

    print("\nRendering task-relevant line charts ...")
    plot_task_relevant_line_chart(
        results, "peg_touch_start", "val_auc",
        "peg touching start_block at t+8 (classification)",
        os.path.join(task_relevant_dir, "peg_touch_start_val_auc_vs_epoch.png"),
    )
    plot_task_relevant_line_chart(
        results, "ab_touch", "val_auc",
        "task-relevant blocks (A,B) touching at t+8 (classification)",
        os.path.join(task_relevant_dir, "ab_touch_val_auc_vs_epoch.png"),
    )
    plot_task_relevant_line_chart(
        results, "dist_peg_start", "val_r2",
        "dist(peg, start_block) at t+8 (regression)",
        os.path.join(task_relevant_dir, "dist_peg_start_val_r2_vs_epoch.png"),
    )
    plot_task_relevant_line_chart(
        results, "dist_ab", "val_r2",
        "dist(A, B) at t+8 (regression)",
        os.path.join(task_relevant_dir, "dist_ab_val_r2_vs_epoch.png"),
    )
    print(f"Saved 4 task-relevant charts -> {task_relevant_dir}")
