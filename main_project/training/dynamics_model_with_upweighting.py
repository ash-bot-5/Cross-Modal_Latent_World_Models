...
class WeightedLangTableDataset(LangTableDataset):
    """LangTableDataset + a per-sample loss weight: changed_weight if the ground-truth
    touching/peg_touching label differs between t and t+HORIZON, else 1.0."""

    def __init__(self, *args, changed_weight: float = 3.0, **kwargs):
        super().__init__(*args, **kwargs)
        self.changed_weight = changed_weight

    def _is_changed(self, ep_idx: int, t: int) -> bool:
        return bool(
            self.touching[ep_idx][t] != self.touching[ep_idx][t + self.HORIZON]
            or self.peg_touching[ep_idx][t] != self.peg_touching[ep_idx][t + self.HORIZON]
        )
    ...
    def __getitem__(self, idx: int) -> dict[str, torch.Tensor]:
        ...
        item["weight"] = torch.tensor(
            self.changed_weight if self._is_changed(ep_idx, t) else 1.0, dtype=torch.float32
        )
        return item

    def changed_fraction(self) -> float:
        flags = [self._is_changed(ep_idx, t) for ep_idx, t in self.indices]
...
def train(
    ...
    checkpoint_dir: str = "checkpoints_upweighted",
    resume_from: str | None = None,
    n_val_episodes: int = 750,
    val_seed: int = 0,
    changed_weight: float = 3.0,
):
    import wandb

    wandb.init(
        project=wandb_project,
        name=f"upweighted_changed_{changed_weight}x",
        config=dict(
    ...
    train_dataset = WeightedLangTableDataset(
        data_dir, device=device, file_list=train_files, changed_weight=changed_weight
    )
    val_dataset = WeightedLangTableDataset(
        data_dir, device=device, file_list=val_files, changed_weight=changed_weight
    )
    ...
    train_changed_frac = train_dataset.changed_fraction()
    val_changed_frac = val_dataset.changed_fraction()
    print(f"Changed fraction — train: {train_changed_frac:.4f}  val: {val_changed_frac:.4f}")
    wandb.log({
        ...
        "dataset/train_changed_frac": train_changed_frac,
        "dataset/val_changed_frac": val_changed_frac,
    })
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for batch in train_loader:
            ...
            weights = batch["weight"].to(device)  # (B,) — changed_weight or 1.0
        ...
        # ---- Validation pass (unweighted — comparable to the baseline run's val loss) ----
        model.eval()
        val_loss_sum = 0.0
...
if __name__ == "__main__":
    ...
    parser.add_argument("--changed_weight", type=float, default=3.0, help="loss weight for changed transitions")
    args = parser.parse_args()

    if args.train:
        train(resume_from=None, num_epochs=50, changed_weight=args.changed_weight)
    else:
        # Smoke test
        DATA_DIR = "/home/ashwink/full_data_5000/langtable/demos/"
        dataset = WeightedLangTableDataset(DATA_DIR, changed_weight=3.0)
        loader = DataLoader(dataset, batch_size=4, num_workers=0, shuffle=True)
        batch = next(iter(loader))
        for k, v in batch.items():
            print(f"{k}: shape={v.shape}, dtype={v.dtype}")

        print(f"changed_fraction: {dataset.changed_fraction():.4f}")
