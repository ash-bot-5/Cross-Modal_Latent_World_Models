...
def train_val_split(data_dir: str, n_val: int = 750, seed: int = 0) -> tuple[list[str], list[str]]:
    """Reproducible split of episode .pkl filenames into (train_files, val_files)."""
    pkl_files = sorted(f for f in os.listdir(data_dir) if f.endswith(".pkl"))
    rng = random.Random(seed)  # local RNG — doesn't touch global random state
    shuffled = pkl_files[:]
    rng.shuffle(shuffled)
    return sorted(shuffled[n_val:]), sorted(shuffled[:n_val])  # (train_files, val_files)
...
class LangTableDataset(Dataset):
    ...
    def __init__(
        ...
        for pkl_file in pkl_files:
            ...
            z_sem_data = z_sem_npz["z_sem"]           # (T, 768)
            T = len(z_sem_data)
            ...
            self.touching.append(z_sem_npz["touching"])          # (T,) bool
            self.peg_touching.append(z_sem_npz["peg_touching"])  # (T,) bool
            self.unique_zsem.append(torch.tensor(zhard["unique_zsem"], dtype=torch.float32))  # (N, 768)
            self.neg_indices.append(zhard["neg_indices"])  # (T, 448) int16, numpy
    ...
    def __getitem__(self, idx: int) -> dict[str, torch.Tensor]:
        ...
        idx        = self.neg_indices[ep_idx][t + self.HORIZON].astype(np.int64)  # (448,) int64
        z_hard_neg = self.unique_zsem[ep_idx][idx]                # (448, 768)
...
if __name__ == "__main__":
    ...
    assert sample["z_hard_neg"].shape == (448, 768), f"Expected (448, 768), got {sample['z_hard_neg'].shape}"
